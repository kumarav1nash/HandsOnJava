...
function App() {
  ...
  // const [mode, setMode] = useState(localStorage.getItem('app_mode') || 'Compiler') // Replaced by context
  ...
  // const onModeToggle = ... // Replaced by onNavMode
  ...
  const { mode: appMode, toggleMode } = useMode()
  const [localMode, setLocalMode] = useState('Compiler') // Sub-mode for Practice (Compiler/Problems)

  // Routing: sync mode with URL
  const location = useLocation()
  const navigate = useNavigate()

  // Effect to sync URL with App Mode
  useEffect(() => {
    if (appMode === 'learn' && !location.pathname.startsWith('/learn')) {
      navigate('/learn')
    } else if (appMode === 'practice' && location.pathname.startsWith('/learn')) {
      navigate('/')
    }
  ...
  const onNavMode = useCallback((next) => {
    if (next === 'practice' || next === 'learn') {
      toggleMode()
      return
    }
    // Handle sub-navigation in Practice mode
    if (next === 'Admin') {
      navigate('/admin')
      setLocalMode('Admin')
    } else if (next === 'Problems') {
      navigate('/practice/problems')
      setLocalMode('Problems')
    } else if (next === 'Compiler') {
      navigate('/practice/compiler')
      setLocalMode('Compiler')
    }
  }, [toggleMode, navigate])
  ...
  return (
    <div className="app" role="application" aria-label={t('header.title')}>
      {/* Show header only on non-landing pages and non-learn course pages */}
      {!location.pathname.startsWith('/learn/course') && location.pathname !== '/' && (
        <Header
          ...
          lastSaved={lastSaved}
          isOnline={isOnline}
          mode={appMode === 'learn' ? 'Learn' : localMode}
      ...
      <main className="app__main" role="main">
        ...
        {location.pathname === '/' ? (
          ...
          (() => {
            // Practice mode routes - handle compiler, problems, and catalog
            if (location.pathname === '/practice' || location.pathname === '/practice/compiler') {
              return (
                <SplitPane
            ...
            } else if (location.pathname.startsWith('/practice/catalog')) {
              return <ProblemsCatalog />
            } else if (location.pathname.startsWith('/practice/problems')) {
              return <ProblemsSection onProblemNavChange={setProblemNav} />
            }
            // Default to compiler if no specific practice route
            return (
              <SplitPane
                direction="horizontal"
                sizes={[60, 40]}
